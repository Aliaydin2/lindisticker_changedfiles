...
class LindiController extends ChangeNotifier {
  ...
  final void Function(List<List<dynamic>>, int) updateValue;
  ...
  LindiController(
      ...
      required this.oldWidgets,
      // required this.oldWidgets,
      required this.updateValue}) {
    for (var oldWidget in oldWidgets) {
      // Assuming oldWidget structure is [widget, oldScaleValue, oldNotifierValue]
      Key key =
          Key('lindi-${DateTime.now().millisecondsSinceEpoch}-${_nrRnd()}');
      ...
      widgets.add(
        DraggableWidget(
          ...
          maxScale: maxScale,
          scale: oldWidget[1],
          oldNotifierValue: oldWidget[2],
          onBorder: (key) => _border(key),
          onDelete: (key) => _delete(key),
          onLayer: (key) => _layer(key),
          onLayerPlus: (key) => _layerPlus(key),
          child: oldWidget[0],
        ),
      );
      _border(key);
  ...
  // : widgets = List<DraggableWidget>.from(oldWidgets);
  ...
  updateNewWidgets() {
    var newWidgets = <List<dynamic>>[];
    for (var widget in widgets) {
      newWidgets.add([widget.child, widget.scale, widget.notifier.value]);
    }
    updateValue(newWidgets, imageIndex);
  }
